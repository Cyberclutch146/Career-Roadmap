  ...
  return (
    <section id="examples" className="relative min-h-0 md:min-h-[100dvh] w-full flex flex-col justify-center border-b border-white/5 py-16 md:py-24">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 w-full">
        ...
        {!showMobileExample && (
          <div className="flex md:hidden flex-col items-center justify-center p-6 rounded-2xl bg-zinc-900/60 border border-zinc-800 text-center gap-4 mt-2 max-w-md mx-auto">
        ...
        {showMobileExample && (
          <div className="flex md:hidden justify-center mb-6">
            <button
              ...
              className="px-4 py-2 rounded-full border border-zinc-800 text-zinc-400 font-semibold font-label text-xs hover:text-white transition-all active:scale-95"
            >
              Hide Example
            </button>
          </div>
        ...
        <div className={showMobileExample ? 'block' : 'hidden md:block'}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 max-w-5xl mx-auto">
            ...
            <motion.div
              layout
              className="lg:col-span-2 rounded-2xl bg-zinc-900/60 border border-zinc-800 overflow-hidden shadow-2xl flex flex-col justify-between"
            >
              {/* Header info */}
              <div className="p-5 md:p-6 bg-gradient-to-b from-zinc-800/20 to-transparent border-b border-zinc-800">
                <div className="flex items-start justify-between flex-wrap gap-3">
                  <div>
                    <span className="text-[10px] font-label font-bold tracking-widest text-amber-500 uppercase">Live Preview</span>
                    <h3 className="font-headline text-lg sm:text-2xl font-bold text-white mt-1">
                      {activeRoadmap.title}
                    </h3>
                    <p className="font-body text-xs sm:text-sm text-zinc-400 mt-1 max-w-md">
                      {activeRoadmap.description}
                    </p>
                  </div>
                  <div className="flex items-center gap-2 bg-amber-500/10 border border-amber-500/20 rounded-2xl px-4 py-2 font-label">
                    <span className="text-zinc-400 text-xs">Total Progress:</span>
                ...
                <div className="flex flex-wrap items-center gap-3 mt-4 text-zinc-400 text-xs font-label">
                    ...
                    <div key={phase.id} className="space-y-3 relative">
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          <h4 className="font-headline font-bold text-zinc-200 text-sm sm:text-base">{phase.name}</h4>
                        </div>
                        <span className="font-label text-xs sm:text-sm text-zinc-400 font-medium">{phaseProgress}%</span>
                      </div>
                      ...
                      <div className="space-y-2 mt-3 pl-3 border-l border-zinc-800/80 ml-4">
                        {phase.chapters.map(chapter => (
                          <div key={chapter.id} className="space-y-2">
                            <h5 className="font-body text-xs font-bold text-zinc-400 tracking-wide uppercase mt-2">
                              {chapter.title}
                            </h5>
                                  ...
                                  <div
                                    ...
                                    className={`flex items-center justify-between p-3 rounded-xl border transition-all duration-200 cursor-pointer ${
                                      isSelected
                                        ...
                                        : 'bg-zinc-950/20 border-zinc-800/60 hover:bg-zinc-800/30 hover:border-zinc-700/60'
                                    }`}
                                  >
                                    <div className="flex items-center gap-3 min-w-0 flex-1">
                                      ...
                                      >
                                        {isCompleted ? (
                                          ...
                                          <Circle className="w-5 h-5 text-zinc-600 hover:text-amber-500/60 transition-colors" />
                                        )}
                                      </button>
                                    ...
                                    <div className="flex items-center gap-2 flex-shrink-0">
                                      <span className="font-label text-[10px] text-zinc-500 px-2 py-0.5 rounded bg-zinc-900 border border-zinc-800/80">
                                        {lesson.duration}
                                      </span>
                                      <ChevronRight className={`w-3.5 h-3.5 text-zinc-600 transition-transform ${isSelected ? 'translate-x-0.5 text-amber-500' : ''}`} />
                                    </div>
                                  </div>
                                )
                              })}
            ...
            <motion.div
              layout
              className="rounded-2xl bg-zinc-900/60 border border-zinc-800 overflow-hidden shadow-2xl p-5 md:p-6 flex flex-col justify-between h-full bg-gradient-to-br from-zinc-900/80 to-zinc-950/40 relative"
            >
              {/* Glow effect */}
                ...
                {selectedLesson ? (
                  <>
                    ...
                    <div className="space-y-3 pt-3">
                      <span className="text-[11px] font-label font-bold tracking-wider text-zinc-500 uppercase block">Curated Materials:</span>
                      
                      <div className="p-3 bg-zinc-950/40 border border-zinc-800 hover:border-amber-500/20 rounded-xl transition-all duration-200 group flex items-start gap-3">
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center justify-between gap-1">
                            <span className="font-body text-xs font-bold text-zinc-200 group-hover:text-amber-400 transition-colors">AI Lecture Video</span>
                            <ExternalLink className="w-3 h-3 text-zinc-500 flex-shrink-0" />
                          </div>
                          <p className="font-body text-[10px] text-zinc-500 mt-0.5 truncate">Adaptive Masterclass Core Lecture</p>
                        </div>
                      </div>
                      ...
                      <div className="p-3 bg-zinc-950/40 border border-zinc-800 hover:border-amber-500/20 rounded-xl transition-all duration-200 group flex items-start gap-3">
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center justify-between gap-1">
                            <span className="font-body text-xs font-bold text-zinc-200 group-hover:text-orange-400 transition-colors">MDN Architecture Docs</span>
                            <ExternalLink className="w-3 h-3 text-zinc-500 flex-shrink-0" />
                          </div>
                          <p className="font-body text-[10px] text-zinc-500 mt-0.5 truncate">Practical Reference Implementation</p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="bg-zinc-950/60 border border-zinc-800/80 rounded-xl p-3.5 mt-4 space-y-2">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] font-label font-bold text-zinc-400 tracking-wider uppercase">AI Mentor Active</span>
                      </div>
                      <p className="font-body text-[11px] text-zinc-300 italic leading-relaxed">
                  ...
                  <div className="text-center py-12 text-zinc-500 font-body text-sm">
