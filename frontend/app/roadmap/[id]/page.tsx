...
export default function RoadmapPage() {
  ...
  const SidebarContent = () => (
    <div className="p-5">
      ...
      <div className="mb-6">
        <h2 className="font-headline font-bold text-base text-on-surface mb-1.5 leading-snug">
      ...
      <div className="mb-6 p-4 rounded-xl bg-surface-container/80 border border-outline-variant/50">
        <div className="flex justify-between text-sm mb-2.5">
          <span className="text-on-surface-variant text-xs font-medium">Progress</span>
        ...
        <div className="mt-2.5 flex items-center justify-between text-xs text-tertiary">
      ...
      <div className="mb-6 border-t border-outline-variant/50 pt-4">
        <span className="text-[10px] font-semibold text-tertiary uppercase tracking-widest block mb-2.5">
          Export
        </span>
        <div className="grid grid-cols-2 gap-2">
          <button
            onClick={exportToMarkdown}
            className="flex items-center justify-center gap-1.5 text-xs py-2.5 px-2 rounded-xl bg-surface-container/80 border border-outline-variant/50 text-on-surface-variant hover:text-amber-400 hover:border-amber-500/20 transition-all"
          >
      ...
      <nav className="space-y-0.5">
        <span className="text-[10px] font-semibold text-tertiary uppercase tracking-widest block mb-2">
          Sections
        </span>
        {[
          ...
          <a
            key={item.href}
            href={item.href}
            className="block px-3 py-2 text-sm text-on-surface-variant hover:text-zinc-200 hover:bg-zinc-800/40 rounded-lg transition-colors"
          >
            {item.label}
          </a>
        ))}
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0b]">
      ...
      <div className="pt-16 flex">
        <aside className="fixed left-0 top-16 bottom-0 w-72 bg-zinc-950/90 border-r border-outline-variant/40 overflow-y-auto hidden lg:block backdrop-blur-sm">
          <SidebarContent />
        </aside>
          ...
          <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8">
            ...
            >
              {[
                ...
                <div
                  key={stat.label}
                  className="relative rounded-2xl bg-surface-container/60 border border-outline-variant/50 p-4 text-center overflow-hidden group hover:border-outline/50 transition-all"
                >
                  ...
                  <div className="relative">
                    ...
                    <div className="font-bold text-xl text-on-surface font-headline tabular-nums">{stat.value}</div>
                    <div className="text-[11px] text-on-surface-variant font-medium mt-0.5">{stat.label}</div>
                  </div>
                </div>
              ))}
            </motion.div>
                    ...
                    {generatedRoadmap.learning_objectives.map((obj) => (
                      <li key={obj.id} className="flex items-start gap-3">
                        <div className={`w-5 h-5 rounded-full flex items-center justify-center mt-0.5 flex-shrink-0 ${
                          obj.mastered ? 'bg-success text-on-surface' : 'bg-surface'
                        }`}>
