  ...
  return (
    <div className="space-y-5 md:space-y-8">
      <div className="text-center space-y-1.5 md:space-y-2 mb-4 md:mb-8">
        ...
        <p className="text-zinc-400 text-xs md:text-sm">How much time can you dedicate?</p>
      </div>

      <div className="space-y-5 md:space-y-8">
        {/* Daily Hours Slider */}
        <div className="bg-zinc-900/40 border border-zinc-800/50 rounded-xl md:rounded-2xl p-4 md:p-6">
          <div className="flex justify-between items-end mb-4 md:mb-6">
            <div>
              <label className="text-[10px] md:text-sm font-semibold uppercase tracking-wider text-zinc-500 block mb-0.5 md:mb-1">Daily Study Time</label>
              <div className="text-zinc-400 text-[10px] md:text-xs hidden sm:block">Consistent daily effort is key.</div>
            </div>
            <div className="text-2xl md:text-3xl font-headline font-bold text-amber-500">
              {dailyHours} <span className="text-sm md:text-base text-zinc-500 font-normal">hrs</span>
            </div>
          </div>
          ...
          <div className="relative pt-1 md:pt-2 touch-none">
            <input
              ...
              className="w-full h-2.5 md:h-2 bg-zinc-800 rounded-lg appearance-none cursor-pointer accent-amber-500"
              style={{ WebkitAppearance: 'none' }}
            />
            <div className="flex justify-between text-[9px] md:text-[10px] font-mono text-zinc-600 mt-2.5 md:mt-3 px-0.5">
              <span>30 min</span>
              <span>4 hrs</span>
              <span>8 hrs</span>
            </div>
        ...
        <div className="space-y-2.5 md:space-y-4">
          <div className="flex items-center gap-2 mb-1 md:mb-2">
            <CalendarRange className="w-3.5 h-3.5 md:w-4 md:h-4 text-zinc-500" />
            <label className="text-[10px] md:text-sm font-semibold uppercase tracking-wider text-zinc-500">Target Duration</label>
          </div>
            ...
            {DURATION_OPTIONS.map((opt) => {
              ...
              return (
                <button
                  ...
                  `}
                >
                  <span className={`font-semibold text-xs md:text-base mb-0 md:mb-0.5 ${isSelected ? 'text-amber-500' : 'text-zinc-300'}`}>
                    <span className="md:hidden">{opt.label}</span>
                    <span className="hidden md:inline">{opt.fullLabel}</span>
                  </span>
                  <span className={`text-[9px] md:text-[10px] uppercase tracking-wider ${isSelected ? 'text-amber-500/70' : 'text-zinc-600'} hidden md:block`}>
                    {opt.desc}
                  </span>
                </button>
              )
        ...
        <div className="bg-zinc-900/60 border border-zinc-800 rounded-xl p-3 md:p-4 flex items-center justify-between">
          <span className="text-xs md:text-sm text-zinc-400">Estimated Total Effort</span>
          <span className="font-mono font-semibold text-sm md:text-base text-zinc-200">~{totalHours} hours</span>
        </div>
      </div>
