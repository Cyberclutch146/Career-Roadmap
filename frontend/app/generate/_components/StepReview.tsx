...
export function StepReview({
  ...
  const SummaryRow = ({ label, value, step }: { label: string, value: string, step: number }) => (
    <div className="flex items-start justify-between py-2.5 md:py-3 border-b border-zinc-800/50 last:border-0 group">
      <div className="min-w-0 flex-1">
        <div className="text-[9px] md:text-[10px] font-semibold uppercase tracking-wider text-zinc-500 mb-0.5 md:mb-1">{label}</div>
        <div className="text-xs md:text-sm font-medium text-zinc-200 break-words">{value}</div>
      </div>
      <button 
        onClick={() => goToStep(step)}
        className="ml-2 p-1.5 text-zinc-500 hover:text-amber-500 hover:bg-amber-500/10 rounded-md transition-all flex-shrink-0 active:scale-90 md:opacity-0 md:group-hover:opacity-100"
      >
  ...
  return (
    <div className="space-y-5 md:space-y-8">
      <div className="text-center space-y-1.5 md:space-y-2 mb-4 md:mb-8">
        ...
        <p className="text-zinc-400 text-xs md:text-sm">Everything looks good? Ready to build your roadmap.</p>
      </div>

      <div className="space-y-4 md:space-y-6">
        {/* Pre-Assessment Card */}
        <div className="bg-zinc-900/40 border border-zinc-800 rounded-xl md:rounded-2xl p-4 md:p-5 relative overflow-hidden group hover:border-amber-500/30 transition-colors duration-300">
          ...
          <div className="flex flex-col gap-3 md:gap-4 relative z-10">
            <div className="flex items-start justify-between gap-3">
              <div className="space-y-1.5 md:space-y-2 min-w-0 flex-1">
                <h4 className="text-xs md:text-sm font-bold text-zinc-100 flex items-center gap-1.5 md:gap-2 flex-wrap">
                  <Award className="w-3.5 h-3.5 md:w-4 md:h-4 text-amber-500 flex-shrink-0" />
                  Skill Assessment
                  <span className="text-[9px] md:text-[10px] font-normal uppercase tracking-wider text-zinc-500 bg-zinc-900 px-1.5 md:px-2 py-0.5 rounded-full border border-zinc-800">Optional</span>
                </h4>
              ...
              <Button
                ...
                className="text-[10px] md:text-xs py-1.5 md:py-2 px-3 whitespace-nowrap bg-zinc-800 hover:bg-zinc-700 text-zinc-200 border border-zinc-700 flex-shrink-0 active:scale-95"
              >
        ...
        {/* Summary Table */}
        <div className="bg-zinc-900/30 border border-zinc-800/50 rounded-xl md:rounded-2xl p-4 md:p-5">
      ...
      <div className="pt-5 md:pt-8 border-t border-zinc-800/50 flex gap-3 justify-between">
        <Button onClick={() => goToStep(3)} variant="ghost" disabled={isGenerating} className="flex-1 sm:flex-none sm:w-auto text-zinc-400 hover:text-zinc-200 active:scale-95">
          Back
        </Button>
        <Button 
          onClick={onGenerate} 
